...
function principal(){
  /***    ###########################################
    ...
    */
    españa = ciudades.filter(a=>a.nombre.includes("Spain"));

    // Calcula el precio medio de los tomates en Alemania "Germany"
    precio_tomates = ciudades.filter(b=>b.nombre.includes("Germany")).map(y=>parseFloat(y.precio_tomate));
    precio_medio = precio_tomates.reduce((total, m, index, precio_tomates) => {
        total += m;
        if( index === precio_tomates.length-1) { 
        return total/precio_tomates.length;
        }else { 
        return total;
        }
    //usa Map para crear una lista de precios de tipo Float
